...
with schemdraw.Drawing() as d:
    ...
    sn = elm.Dot()
    line_from_sn = elm.Line().left(4.5).at(sn.start)
    ...
    pd = elm.Photodiode().label('PD1', loc='bottom').at(tx.drain).theta(90).anchor('end')
    elm.Ground().at(pd.start)
    tx = elm.AnalogNFet(offset_gate=False, arrow=False).anchor('drain').theta(90).label('TX2', loc='right').at(tx.source)
    pd = elm.Photodiode().label('PD2', loc='bottom').at(tx.source).theta(90).anchor('end')
    elm.Ground().at(pd.start)
    ...
    # pixel 2
    elm.Line().down(2).at(pixel_common.start)
    elm.Line().left(0.1)
    tx = elm.AnalogNFet(offset_gate=False, arrow=False).anchor('source').theta(90).label('TX3', loc='right')
    pd = elm.Photodiode().up().label('PD3', loc='bottom').at(tx.drain).theta(90).anchor('end')
    elm.Ground().at(pd.start)
    tx = elm.AnalogNFet(offset_gate=False, arrow=False).anchor('drain').theta(90).label('TX4', loc='right').at(tx.source)
    pd = elm.Photodiode().label('PD4', loc='bottom').at(tx.source).theta(90).anchor('end')
    elm.Ground().at(pd.start)
